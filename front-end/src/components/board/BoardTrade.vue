...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'tradeState', label: '거래상태', tdClass: 'tdClass' },
      ],
    };
  },
  created() {
    axios.get(`http://192.168.214.52/board?boardType=2&key=${this.key}&word=${this.word}`).then((resp) => {
  ...
  methods: {
    ...
    mvRegist() {
      localStorage.setItem('registType', 2);
      this.$router.push({
        name: 'regist',
      });
    },
